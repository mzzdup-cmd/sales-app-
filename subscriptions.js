class SubscriptionsManager {
    ...
    createSubscriptionRow(subscription) {
        ...
        tr.innerHTML = `
            ...
            <td>${subscription.firstPaymentDate ? new Date(subscription.firstPaymentDate).toLocaleDateString() : '-'}</td>
            <td>
    ...
    calculateFuturePayments(subscription) {
        ...
        for (let i = 1; i <= 3; i++) {
            if (!payments[`payment${i}`]) total += amount * 1000;
        }
        return total;
    }
    ...
    async togglePayment(subscriptionId, paymentNumber, isPaid) {
        await db.collection("subscriptions")
            .doc(subscriptionId)
            .update({ [`payments.payment${paymentNumber}`]: isPaid });
    ...
    async checkSubscriptionStatus(subscriptionId) {
        ...
        const allPaid = Object.keys(payments).length > 0 && Object.values(payments).every(v => v);
        if (allPaid) {
            await db.collection("subscriptions").doc(subscriptionId).update({ status: "Оплачено" });
            return;
        }
        ...
        for (let i = 0; i < dates.length; i++) {
            ...
            if (!payments[key] && dates[i] < today) {
                await db.collection("subscriptions").doc(subscriptionId).update({ status: "Слив" });
                break;
            }
        }
    }
    ...
    async addSubscription(clientNick = "Новый клиент") {
